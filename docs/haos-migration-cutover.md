...
## Upgrade Note (Legacy Broken Entry -> Latest Release)

If you are upgrading from an older broken install, update to the latest release
before troubleshooting auth.
...
1. Update add-on + integration to the latest release.
...
- Ensure installed integration version matches latest release

Validation (must pass):
- Integration files on disk are updated
- No duplicate old copy remains in another path
- `custom_components/psegli/manifest.json` version matches the latest release
